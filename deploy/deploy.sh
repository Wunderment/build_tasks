...
for DEVICE in $PROCESS_DEVICES; do
	...
	if [ -f $PKGNAME.zip ]; then
		...
		lftp sftp://$WOS_USER:$WOS_PASS@$WOS_HOST -e "set sftp:auto-confirm yes; cd $WOS_DIR_FULL; put $PKGNAME.zip -o $PKGNAME.piz; put $PKGNAME.zip.md5sum; put $PKGNAME.zip.prop; mv $PKGNAME.piz $PKGNAME.zip; rm WundermentOS-$LOS_DEVICE-latest.zip; ln -s $PKGNAME.zip WundermentOS-$LOS_DEVICE-latest.zip; bye"
